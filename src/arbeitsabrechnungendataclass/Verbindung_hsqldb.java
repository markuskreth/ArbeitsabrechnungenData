...
public class Verbindung_hsqldb {
    Logger logger = Logger.getLogger("ArbeitrechnungenDataClass.Verbindung_hsqldb");
     ...
     * Datenbank wird im Arbeitsverzeichnis im Unterverzeichnis "daten" gespeichert
     */
	...
	sql("CREATE TABLE klienten_angebote ("
	...
	sql("CREATE TABLE rechnungen_einheiten ( "
	...
	// Programmverzeichnis nutzen um Datenbank anzulegen
        logger.setLevel(Level.DEBUG);
	String datenbank = pfad + sysProps.getProperty("file.separator") + "daten"
		+ ";shutdown=true";

	URL += datenbank;
	
	logger.debug("Pfad zur Datenbank:");
	logger.debug(datenbank);
	...
	try {
            ...
            logger.debug("JDBC/MySql-Treiber konnte nicht geladen werden!");
            return;
        }
        // Verbindung aufbauen
